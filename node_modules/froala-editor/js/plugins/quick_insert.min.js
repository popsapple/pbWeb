    ...
    d.extend(d.FE.DEFAULTS, {
        ...
        function n(e) {
            a || function() {
                ...
                }, !0), r.events.on("commands.before", s), r.events.on("commands.after", function() {
                    r.popups.areVisible() || i()
                }), r.events.bindClick(r.$box, ".fr-quick-insert > a", o), r.events.bindClick(r.$box, ".fr-qi-helper > a.fr-btn", function(e) {
                    var t = d(e.currentTarget).data("cmd");
                    if (!1 === r.events.trigger("quickInsert.commands.before", [t])) return !1;
                    d.FE.QUICK_INSERT_BUTTONS[t].callback.apply(r, [e.currentTarget]), d.FE.QUICK_INSERT_BUTTONS[t].undo && r.undo.saveStep(), r.events.trigger("quickInsert.commands.after", [t]), r.quickInsert.hide()
                }), r.events.$on(r.$wp, "scroll", function() {
                    a.hasClass("fr-visible") && t(a.data("tag"))
                })
            }(), a.hasClass("fr-on") && u(), r.$box.append(a), t(e), a.data("tag", e), a.addClass("fr-visible")
        }

        function i() {
            if (r.core.hasFocus()) {
                var e = r.selection.element();
                if (r.opts.enter == d.FE.ENTER_BR || r.node.isBlock(e) || (e = r.node.blockParent(e)), r.opts.enter == d.FE.ENTER_BR && !r.node.isBlock(e)) {
                    var t = r.node.deepestParent(e);
                    t && (e = t)
                }
                e && (r.opts.enter != d.FE.ENTER_BR && r.node.isEmpty(e) && r.node.isElement(e.parentNode) && 0 <= r.opts.quickInsertTags.indexOf(e.tagName.toLowerCase()) || r.opts.enter == d.FE.ENTER_BR && ("BR" == e.tagName && (!e.previousSibling || "BR" == e.previousSibling.tagName || r.node.isBlock(e.previousSibling)) || r.node.isEmpty(e) && (!e.previousSibling || "BR" == e.previousSibling.tagName || r.node.isBlock(e.previousSibling)) && (!e.nextSibling || "BR" == e.nextSibling.tagName || r.node.isBlock(e.nextSibling)))) ? a && a.data("tag").is(d(e)) && a.hasClass("fr-on") ? u() : r.selection.isCollapsed() && n(d(e)) : s()
            }
        }
            ...
            if (e && e.preventDefault(), a.hasClass("fr-on") && !a.hasClass("fr-hidden")) u();
                ...
                if (!r.shared.$qi_helper) {
                    for (var t = r.opts.quickInsertButtons, n = '<div class="fr-qi-helper">', i = 0, s = 0; s < t.length; s++) {
                        ...
                        o && (!o.requiredPlugin || d.FE.PLUGINS[o.requiredPlugin] && 0 <= r.opts.pluginsEnabled.indexOf(o.requiredPlugin)) && (n += '<a class="fr-btn fr-floating-btn" role="button" title="' + r.language.translate(o.title) + '" tabIndex="-1" data-cmd="' + t[s] + '" style="transition-delay: ' + .025 * i++ + 's;">' + r.icon.create(o.icon) + "</a>")
                    }
                    n += "</div>", r.shared.$qi_helper = d(n), r.tooltip.bind(r.shared.$qi_helper, "> a.fr-btn"), r.events.$on(r.shared.$qi_helper, "mousedown", function(e) {
                        e.preventDefault()
                    }, !0)
                }(l = r.shared.$qi_helper).appendTo(r.$box), setTimeout(function() {
        ...
        return {
            _init: function() {
                if (!r.$wp) return !1;
                r.opts.iframe && r.$el.parent("html").find("head").append('<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.4.0/css/font-awesome.min.css">'), r.popups.onShow("image.edit", s), r.events.on("mouseup", i), r.helpers.isMobile() && r.events.$on(d(r.o_doc), "selectionchange", i), r.events.on("blur", s), r.events.on("keyup", i), r.events.on("keydown", function() {
                    setTimeout(function() {
                        i()
                    }, 0)
                })
